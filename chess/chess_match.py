...
class ChessMatch:
    ...
    # Returns the king chess piece by color
    def king(self, color):
        filtered_list = list(filter(lambda el: el.get_color() == color, self.pieces_on_the_board))
        for el in filtered_list:
    ...
    # Returns the possible moves by position
    ...
    # Execute the move
    def perform_chess_move(self, source, target, is_it_position=False):
        if not is_it_position:
            source = source.to_position()
            target = target.to_position()
        self.validate_source_position(source)
    ...
    def setup(self):
        ...
        self.place_new_piece("d", 1, Queen(self.board, "WHITE"))
        self.place_new_piece("e", 1, King(self.board, "WHITE"))
        ...
        self.place_new_piece("d", 8, Queen(self.board, "BLACK"))
        self.place_new_piece("e", 8, King(self.board, "BLACK"))
